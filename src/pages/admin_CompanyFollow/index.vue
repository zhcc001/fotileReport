            ...
            <div class="scroll-list-wrap saleWarp">
              <cube-scroll ref="scrollSale" :data="Status" :options="saleOptions">
                <ul class="selectListPerson">
                  ...
                  <li v-for="(item,index) in Person" :key='index' :dataZM='item.ZM' :class="{active:index==personHasActive}">
                    <span>{{item.Name}}</span>
          ...
          <div class="contentList" v-for="(item,index) in list" :key="index" @click="companyDetail(item.ID)">
            <div class="contentListTop">
              <p class="firstLine">
                ...
                <span v-if="AccessId==5&&(item.Status==1||item.Status==2)">剩余保护期:{{item.EndDate}}天</span>
                <span v-if="AccessId==5&&(item.Status==3||item.Status==-3)">合同剩余时间:{{item.EndTime}}天</span>
              </p>
              <p class="twoLine">

                <a href="javascript:;" class="round" :class="{'active':checkBoxs[index]}" @click.stop="check(index,item.ID)"
                  v-if='postShow'><b :class="{'active':checkBoxs[index]}"></b></a>
                <!-- 公司评级 -->
              ...
              <p class="downLine"><span class="zuZhi">{{item.SalesManName==''?'暂无业务员':item.SalesManName}}</span><b v-if='AccessId==5'>{{item.SalesManName==''?'暂无地址':item.Organization}}</b></span><span
    ...
    methods: {
      ...
      sureGroup() {
        if (this.salesNum > 0) {
          ...
          this.groupSelect = this.personSelect
